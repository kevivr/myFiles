...
public class Gradebook {
  ...
  public List<Double> getFinalScores(List<Double> weights) {
    return gradebook.stream().map(s -> s.getGrade(weights)).collect(Collectors.toList());
  }
  ...
  public int countLetterGrade(String grade, List<Double> weights) {
    return (int) gradebook.stream()
            .map(s -> s.getGrade(weights)).filter(p -> lettergrade(p).equals(grade)).count();
  }
  ...
  public Double averageScoreForName(String name, List<Double> weights) {
    return gradebook.stream().filter(p -> p.getFirstName().equals(name))
            .mapToDouble(s -> s.getGrade(weights)).average().orElse(0.0);
  }
  ...
  public long countAboveAverage(List<Double> weights) {
    return gradebook.stream().filter(s -> s.getGrade(weights)
            > gradebook.stream().mapToDouble(p -> p.getGrade(weights)).average()
            .orElseThrow(RuntimeException::new)).count();
  }
